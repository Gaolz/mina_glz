...
namespace :puma do
  ...
  task start: :environment do
    ...
    queue! %(
      if [ -e '#{puma_pid}' ]; then
        echo 'Puma is running, if want restart, just use "mina puma:restart"';
      else
  ...
  task stop: :environment do
    ...
    queue! %(
      if [ -e '#{puma_pid}' ]; then
        ...
        echo 'stop puma service successfully';
      fi
    )
  end
  ...
  task restart: :environment do
    ...
    queue! %(
      if [ -e '#{puma_pid}' ]; then
        ...
        echo "send USR2 signal of restarting to puma service."
      else
        echo "puma is not running, now starting puma service."
        cd #{deploy_to}/#{current_path} && #{puma_cmd} -C #{puma_config}
      fi
    )
  end
